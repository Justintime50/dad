...
export async function createSlimJson() {
  ...
  try {
    for (let i = 0; i < numberOfAddresses; i++) {
      const fileContent = await promises.readFile(dataSetFilePath, 'utf-8');
      const dataSetFile = JSON.parse(fileContent).addresses;
    ...
    await promises.writeFile(outputPath, JSON.stringify(finalList) + '\n');
    console.log(
      `Addresses slimmed and saved to ${outputPath}. Place the new list in its corresponding country folder and rename as needed.`
    );
    return true;
  } catch (error) {
    throw Error(error);
